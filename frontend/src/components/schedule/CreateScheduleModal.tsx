...
const CreateScheduleModal = ({
  ...
  const [formData, setFormData] = useState({
    name: '',
    testType: 'case', // 'case' or 'file'
    testCaseId: '',
    testFileId: '',
    scheduleType: 'one-time', // 'one-time', 'daily', 'weekly', 'monthly'
    scheduledDate: '',
    scheduledTime: '09:00',
    timezone: 'UTC',
    recurrencePattern: {
      days: [] as number[], // For weekly: 0=Sun, 1=Mon, etc.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.testType === 'case' && !formData.testCaseId) {
      setError('Please select a test case');
      return;
    }

    if (formData.testType === 'file' && !formData.testFileId) {
    ...
    try {
      ...
      // User selects 14:00 local → Convert to UTC (12:00 if UTC+2)
      ...
      if (formData.testType === 'case') {
        payload.testCaseId = formData.testCaseId;
      } else {
        payload.testFileId = formData.testFileId;
      }
    ...
    setFormData(prev => ({
      ...
      recurrencePattern: {
        ...
        days: prev.recurrencePattern.days.includes(day)
          ? prev.recurrencePattern.days.filter(d => d !== day)
            ...
            <div className="flex space-x-4 mb-3">
              <label className="flex items-center">
                <input
                  type="radio"
                  value="case"
                  checked={formData.testType === 'case'}
                  onChange={(e) => setFormData({ ...formData, testType: e.target.value })}
                  className="mr-2"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">Test Case</span>
              </label>
              <label className="flex items-center">
                <input
                  type="radio"
                  value="file"
                  checked={formData.testType === 'file'}
                  onChange={(e) => setFormData({ ...formData, testType: e.target.value })}
                  className="mr-2"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">Test File</span>
              </label>
            </div>

            {formData.testType === 'case' ? (
              <select
